...
def preview_stem(voice_id: str) -> str:
    """What a preview of `voice_id` is filed under.

    The narration version is part of the name for the same reason it is part
    of a narration node's hash: bumping it means the synthesizer produces
    different audio for identical params, and a cache keyed on the voice
    alone would go on serving the old wav forever. That is exactly the drift
    this route exists to prevent — the picker would audition v2 while every
    project rendered v3.
    """
    return f"{voice_id}.v{NARRATION_VERSION}"
...
class KokoroBackend(ExecutionBackend):
    ...
    async def render_preview(self, voice_id: str, out_dir: Path, stem: str | None = None) -> Path:
        """`PREVIEW_LINE` in one voice, published in `out_dir`.

        `stem` names the file, defaulting to the versioned `preview_stem`
        the engine's own cache is keyed on. The one caller that overrides it
        is the swatch-sample script: those five wavs are bundled with the
        desktop under a bare `<id>.wav` that a contract test pins, and they
        are regenerated by hand rather than cached, so the version that
        makes the engine's cache correct would only break their names.
        ...
        spec = JobSpec(
            ...
            output_hash=stem if stem is not None else preview_stem(voice_id),
            # `narration_version` travels for the reason it is on every
            # narration node: it does not change what is synthesized, it
            # addresses what was. Carrying it keeps the params a preview is
            # made from the same shape as the ones a project renders from.
            params={
                "text": PREVIEW_LINE,
                "voice_id": voice_id,
